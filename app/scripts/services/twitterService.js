   ...
   return {
      ...
      getUser: function() {
         ...
         var promise = TwitterAuth.get('/1.1/account/verify_credentials.json').then(function(data) {
            ...
            console.log('waiting...');
            $timeout(getAuthUser, 30000); // Wait 5 minutes for new session
         });
         ...
         function getFollowers(cursor) {
            ...
            if (cursor || cursor === 0) {
               ...
               promise.then(function(data) {
                  ...
                  // If session has expired
                  console.log('waiting...');
                  $timeout(getFollowers, 30000); // Wait 5 minutes for new session
               });
            } else {
         ...
         if (maxId) {
            ...
            if (i < 3) { // i<16 to get up to 3200 last tweets
               ...
               promise.then(function(data) {
                  ...
                  console.log('waiting…');
                  $timeout(getTweet(), 30000); // Wait 5 minutes for new session
               });
               return defer.promise;
            } else {
